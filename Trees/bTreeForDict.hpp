...
public:
    bTreeForDict(unsigned, int (*)(const TKey &, const TKey &));
...
template<typename TKey, typename TElement>
bTreeForDict<TKey, TElement>::bTreeForDict(unsigned t, int (*compare)(const TKey&, const TKey&)) {
    minDegree = t;
    comparator = compare;
...
SimplePair<typename bTreeForDict<TKey, TElement>::BNode<SimplePair<TKey, TElement>>*, unsigned> bTreeForDict<TKey, TElement>::search(TKey k) {
    ...
    while(true) {
        ...
        if (i < x->size && !(-1 == comparator(k, x->key[i].first) || -1 == comparator(x->key[i].first, k))) {
            ...
            return SimplePair<BNode<TKey, TElement>*, unsigned>(nullptr, 0);
        }
